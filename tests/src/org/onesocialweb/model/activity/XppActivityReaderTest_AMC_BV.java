...
public class XppActivityReaderTest_AMC_BV {
	...
	public void testParse() {
		...
		try {
            ...
            xpp.setInput(getClass().getClassLoader().getResourceAsStream("atom-activity-martin-atkins(editted).xml"), "UTF-8");
		...
		assertEquals("Entry#3's title is not parsed as expected", 
				new String("Martin Atkins posted 'Only in SF'"), 
				feed.getEntries().get(2).getTitle().getValue());
		assertEquals("Entry#4's title is not parsed as expected", 
				new String("Martin Atkins added 'Hipster overload - sidewalk sunbathing' as a favorite"), 
